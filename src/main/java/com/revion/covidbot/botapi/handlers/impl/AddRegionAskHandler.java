...
@Component
public class AddRegionAskHandler implements InputMessageHandler {
    ...
    public SendMessage handle(Message message) {
        userDataCache.setUserCurrentBotState(message.getFrom().getId(), BotState.ADD_REGION);
        return messagesService.getQuestionEmojiMessage(String.valueOf(message.getChatId()), "reply.region.choose");
    }

    @Override
    public BotState getHandlerName() {
        return BotState.ASK_TO_CHOOSE_REGION_TO_ADD;
    }
