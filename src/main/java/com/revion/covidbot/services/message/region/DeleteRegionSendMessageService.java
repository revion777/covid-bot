...
@Service
@Slf4j
public class DeleteRegionSendMessageService extends RegionSendMessageService {

    public DeleteRegionSendMessageService(RegionService regionService, UserService userService, UserDataCache userDataCache,
    ...
    SendMessage generateSpecificReplyToUser(UserEntity user, RegionEntity region) {
        ...
        if (res) {
            return messageBuilder.getSuccessEmojiMessage(String.valueOf(user.getChatId()), "reply.region.deleted");
        }

        log.info(LogMessage.REGION_ALREADY_UPDATED, user.getId(), region.getTitle());
        return messageBuilder.getInfoReplyMessage(String.valueOf(user.getChatId()), "reply.region.alreadyDeleted");
    }
